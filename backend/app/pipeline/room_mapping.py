...
Rules:
- Group rooms that refer to the same physical space (e.g. "Bathroom" ↔ "Hall Bathroom", "Bedroom 1" ↔ "Bedroom").
- A group can have multiple rooms on either side if they map to the same space.
- Every room from both lists must appear in exactly one group.
- If a room has no match on the other side, put it in a group by itself with an empty list for the other side.
